...
def print_note(index: int, current_user: NoteUser):
    ...
    if note['id'] not in current_user.notes_ids:
        print_error("User can no access user this note")
        return
...
def main():
    ...
    while True:
        ...
        if command == 'exit':
            ...
            if len(args) < 1:
                print_error("Pleas enter a note number")
                continue
            index = int(args[0]) - 1
            if index < 0 or index >= len(note_list):
                print_error("Pleas enter a valid note number")
                continue
